...
def _process_shard_batch(
    ...
    List[Tuple[int, Optional[str], List[str], List[int]]],  # (page_id, title, paragraphs, token_counts)
    ...
    articles: List[Tuple[int, Optional[str], List[str], List[int]]] = []
    ...
    def consumer():
        while True:
            ...
            try:
                ...
                # Compute token counts for paragraphs
                token_counts = [len(tokenize(paragraph)) for paragraph in paragraphs]
                if len(token_counts) != len(paragraphs):
                    logger.error("Token counts mismatch for article %d: %d paragraphs, %d counts", page_id_int, len(paragraphs), len(token_counts))
                    with errors_lock:
                        parsing_errors.append(ValueError(f"Token counts mismatch for article {page_id_int}"))
                    continue

                # Create result tuple
                article_tuple = (page_id_int, title, paragraphs, token_counts)
...
class Command(BaseCommand):
    ...
    def _run_pipeline(
        ...
        article_tuples: List[Tuple[int, Optional[str], List[str], List[int]]] = []
        ...
        def flush_articles_sync(tuples_to_flush: List[Tuple[int, Optional[str], List[str], List[int]]]) -> Tuple[int, int]:
            ...
            unique_tuples: List[Tuple[int, Optional[str], List[str], List[int]]] = []
            ...
            if unique_tuples:
                ...
                with transaction.atomic():
                    with connection.cursor() as cursor:
                        ...
                        with cursor.copy(
                            "COPY search_engine_article (page_id, title, plain_text_paragraphs, paragraph_token_counts, is_disambiguation) FROM STDIN"
                        ) as copy:
                            for page_id, title, paragraphs, token_counts in unique_tuples:
                                copy.write_row([page_id, title, Json(paragraphs), Json(token_counts), False])
